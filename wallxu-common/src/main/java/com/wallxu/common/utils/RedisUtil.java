...
import com.wallxu.common.config.RedisConfig;
...
/**
 * Redis 工具类
 * Created by shupinyougou on 2016/11/26.
 */
public class RedisUtil {
	...
	// 访问密码
//	private static String PASSWORD = com.pyg.common.util.AESUtil.aesDecode(PropertiesFileUtil.getInstance("redis").get("master.redis.password"));
...
//					jedis.auth(PASSWORD);
	...
	public synchronized static String watch(String... keys) {
		Jedis jedis = getJedis();
		if (null == jedis) {
			return null;
		}
		String value = jedis.watch(keys);
		jedis.close();
		return value;
	}

	public synchronized static Transaction multi() {
		Jedis jedis = getJedis();
		if (null == jedis) {
			return null;
		}
	...
	public static void main(String[] args) {
		...
		for (int i=0; i< 1000; i++){
			...
			jedis.watch("shangping12");
			Transaction tx = jedis.multi();
			//有库存，减
			tx.hset("shangping12", "user_id_"+i, "i" );
			List<Object> exec = tx.exec();

			System.out.println(jedis.hlen("shangping12"));
